...
//동적으로 받진 않고 앱시작할때 무조건 다 받아버린다.

public class PermissionRequester {
    //요청한 스마트폰 버전이 마시멜로우 이상 버전이 아닐 경우
    public static final int NOT_SUPPORT_VERSION=2;
    //요청한 권한을 이미 가지고 있는 경우
    ...
    public int request(final String permission, final int requestCode, final OnClickDenyButtonListener denyAction){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            ...
            //리턴결과는 PackageManager.PERMISSION_DENIED 와 PackageManager.PERMISSION_GRANTED로 나뉘어짐
            ...
            if(permissionCheck== PackageManager.PERMISSION_DENIED){
                //권한을 취득했을때 사용자로부터 확인을 받아야 하는지 확인
                //여기서 true가 나올 경우는 해당 앱에서 한번이라도 권한을 Deny한 경우일때 말고는 없음
                //권한에 대해서 허가하지 않은 경우 다시 한번 권한의 취득을 위해 사용자에게 이유를 고지해야함
                //mashmellow 버전 이상부터 사용가능함
                if(context.shouldShowRequestPermissionRationale(permission)){
                    //권한 취득해야 하는 이유를 Dialog 등 통해서 알린다.
                    ...
                    dialog.setTitle(builder.getTitle())
                            ...
                            .setPositiveButton(builder.getPositiveButtonName(), new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    ...
                                    //뒤에 들어가있는 파라미터(requestCode)는 onRequestPermission()에서 권한 취득 결과엣 사용된다.
                    ...
                    //권한의 취득을 요청한다
                    //취득하고자 하는 권한을 배열에 넣고 요청한다.
                    //뒤에 들어가는 파라미터(1000)는 onRequestPermissionsRessult()에서 권한 취득 결과에서 사용된다
                    //startActivityForResult의 RequestCode와 유사하다.
